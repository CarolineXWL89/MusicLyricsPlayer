...
//        int checkPointIndex = 0;
        int endSectionPointsReset = 0;
        for (int i = 0; i < l - SECTION_END_INDICATOR.length(); i++) {
            if(i + VERSE_BREAK.length() < l){
                ...
                iPlaceHolder = i;
                int k = i;
                if(checkPoint.equals(VERSE_BREAK)){
                    while(!(realCheckPoint.equals(SECTION_END_INDICATOR))) {
                        if(k <= l - SECTION_END_INDICATOR.length()){
                            realCheckPoint = fullSong.substring(k, k + SECTION_END_INDICATOR.length());
                            k++;
                        }
                    }
                    numberOfSections++;
